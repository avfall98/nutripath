          ...
          {isReal && entries.length === 0 && (
            <>
              <button
                ...
                aria-label="Add food"
                className="absolute left-1/2 top-1/2 flex size-9 -translate-x-1/2 -translate-y-1/2 shrink-0 items-center justify-center rounded-full border border-dashed border-white/15 text-muted-foreground transition-colors hover:border-primary hover:text-primary md:hidden"
              >
              ...
              <button
                ...
                onClick={() => setAddOpen(true)}
                aria-label="Add food"
                className="absolute left-1/2 top-1/2 hidden size-8 shrink-0 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border border-white/40 text-white/80 transition-colors hover:border-primary hover:text-primary md:flex mt-4"
              >
                <Plus className="size-4" />
              </button>
            </>
      ...
      {entries.length > 0 && (
        <ul className="flex flex-col">
          {entries.map((entry, i) => {
            ...
            const roundedQty = round(entry.quantity, 1)
            const qtyLabel = roundedQty % 1 === 0 ? String(Math.floor(roundedQty)) : String(roundedQty)
            return (
              <li key={entry.id}>
                {/* Desktop table row */}
